...
export default function useAssignmentByNameQuery(name: Ref<string>, districs: Ref<string[]>, adminId: string | null) {
  return useQuery({
    enabled: false,
    ...
    queryFn: async () => {
      ...
      if (!normalizedName || districs.value.length <= 0) return [];
