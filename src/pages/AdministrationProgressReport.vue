        ...
        <template v-else>
          <div class="flex justify-content-between align-items-center">
            <div class="flex flex-column align-items-start mb-4 gap-2">
              <div>
                <div class="uppercase font-light text-gray-500 text-md">Assignment Progress Report</div>
          ...
          <div v-if="!progressUsers?.length || !totalChartStats" class="empty-user-list">
            <div class="text-lg font-semibold text-gray-700">Could not find users for this assignment.</div>
            <div class="mt-2 text-sm text-gray-500">
              <a href="/add-users">Add users</a> to this assignment to see the progress report.
            </div>
          </div>
          ...
          <div v-else>
            <div class="flex flex-column align-items-around flex-wrap gap-3 rounded bg-gray-100 p-2 details-card">
              ...
              <div class="flex flex-column align-items-center mx-5">
                <div class="flex flex-wrap justify-content-around align-items-center gap-3 px-2 py-1 rounded">
            ...
            >
              ...
              <template #expansion="{ data: row }">
                <PvDataTable :value="row.tasks" :row-hover="false" class="p-datatable-sm" size="small">
...
const fetchProgressData = async () => {
  ...
  try {
    const [admin, progress] = await Promise.all([
      administrationsRepository.fetchAdministrationById(props.administrationId),
      administrationsRepository.getAdministrationProgress({
...
watch(
  [initialized, () => props.administrationId],
  () => {
...
const exportAll = async () => {
  ...
  const formattedFileName = `progress-report-${_kebabCase(administrationTitle)}.csv`;
